...
HEM.setup = function(wd=NULL) {
  ...
  if(is.null(wd)) wd <- "C:/main/HEM/RPGen"
  setwd(wd)
  inpath   <- paste0(wd,"/input/")
  outpath  <- paste0(wd,"/output/")
  run      <- "runfile.txt"
  HEMpums  <- "PUMS_reg.csv"
  HEMahs   <- "AHS2019.csv"
  HEMrecs  <- "RECS2019.csv"
  states   <- "states.txt"
  rawAHSh  <- "AHSh.csv"
  rawAHSp  <- "AHSp.csv"
  rawRECS  <- "RECS.csv"
  ...
  library("truncnorm")
  library("survey")
  source("RPGen_control_2019.R")
  source("RPGen_popgen_2019.R")
  source("RPGen_httk_2019.R")
  source("RPGen_housing_2019.R")
}    
