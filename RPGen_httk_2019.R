...
# Based on code developed at EPA, available as the "httk" R package
# Last modified by WGG at ICF, on July 15, 2019
...
estimate_gfr = function (gfrtmp.dt) 
{
  ...
  if (gfrtmp.tmp[, any(age_years >= 12)]) {
    gfrtmp.tmp[age_years >= 12, `:=`(log_serum_creat, predict(sc_spline[[1]], x = age_months)$y +
                                       rfun2(n = sum(age_years >= 12), fhat = sc_kde[[1]])), by = list(gender, reth)]
...
rfun1 = function (n, fhat) 
{
  q.rfun1 <- get.randoms("rfun1",n,g$seeds,g$var.list,0)
  tmp <- fhat$x[sample(seq_along(fhat$x), size = n, replace = TRUE, prob = fhat$w)] + fhat$h * pnorm(q.rfun1, mean = 0, sd = 1)
  return(tmp)
}


rfun2 = function (n, fhat) 
{
  q.rfun2 <- get.randoms("rfun2",n,g$seeds,g$var.list,0)
  tmp <- fhat$x[sample(seq_along(fhat$x), size = n, replace = TRUE, prob = fhat$w)] + fhat$h * pnorm(q.rfun2, mean = 0, sd = 1)
  return(tmp)
}
...
estimate_hematocrit = function (hcttmp_dt)   {
  ...
  if (hcttmp_tmp[, any(age_years >= 1)]) {
    hcttmp_tmp[age_years >= 1, `:=`(log_hematocrit, predict(hct_spline[[1]], x = age_months)$y + 
                                      rfun1(n = sum(age_years >= 1), fhat = hct_kde[[1]])), by = list(gender, reth)]
...
hematocrit_infants = function (age_months) {
  ...
  q.hema <- get.randoms("hema",length(age_months),g$seeds,g$var.list,0) 
  if (length(loghct[age_months < 1]) > 0) {
    sig.tmp <- (log(49) - log(31))/2
    mu.tmp <- log(49) - sig.tmp
    loghct[age_months < 1] <- pnorm(q.hema[age_months < 1], mean = mu.tmp, sd = sig.tmp)
  }
  if (length(loghct[age_months >= 1 & age_months <= 6]) > 0) {
    sig.tmp <- (log(42) - log(29))/2
    mu.tmp <- log(42) - sig.tmp
    loghct[age_months>=1 & age_months<=6] <- pnorm(q.hema[age_months>=1 & age_months<=6], mean=mu.tmp, sd=sig.tmp)
  }
  if (length(loghct[age_months > 6 & age_months < 12]) > 0) {
    sig.tmp <- (log(38) - log(33))/2
    mu.tmp <- log(38) - sig.tmp
    loghct[age_months>6 & age_months<12] <- pnorm(q.hema[age_months>6 & age_months<12], mean=mu.tmp, sd=sig.tmp)
  }
  hct <- exp(loghct)
  return(hct)
}
